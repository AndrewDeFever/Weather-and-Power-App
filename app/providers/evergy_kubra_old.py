...
def fetch_current_state(stormcenter_id: str, view_id: str, debug: bool = False) -> Dict[str, Any]:
    s = _session()
    url = (
...
def _normalize_record(rec: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    return {
        ...
        "n_out": n_out,
        "etr": _safe_str(desc.get("etr")),  # ISO string OR "ETR-EXP"
...
def fetch_evergy_outages(
    ...
    cs = fetch_current_state(stormcenter_id, view_id, debug=debug)
    ...
    for z in zoom_candidates:
        ...
        if cached_scheme:
            ...
            break
        try:
            scheme = discover_tile_scheme(cluster_template, qks_to_try, z, debug=debug)
    ...
    for qk in queue_tiles:
        ...
        new_outs = fetch_tile_records(cluster_template, scheme, qk, debug=debug)
        for o in new_outs:
    ...
    while drill_queue:
        ...
        for tqk in tiles_next:
            ...
            new_outs = fetch_tile_records(cluster_template, scheme, tqk, debug=debug)
            for o in new_outs:
                if o.get('cluster') is True:
